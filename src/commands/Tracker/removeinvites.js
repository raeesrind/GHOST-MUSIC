...
module.exports = {
  ...
  category: 'Tracker',
  usage: 'removeinvites <user> <amount> <total|fake>',
  userPerms: ['ManageGuild'],
  slashOptions: [
    { name: 'user', description: 'The user', type: 6, required: true },
    { name: 'amount', description: 'Number of invites to remove', type: 4, required: true },
  ...
  async slashExecute(interaction, client) {
    ...
    if (type === 'fake') {
      ...
      const newFake = Math.max(0, (current.fake || 0) - amount);
      client.db.invites.set(interaction.guild.id, user.id, { fake: newFake });
    } else {
      const current = client.db.invites.get(interaction.guild.id, user.id) || { invites: 0, bonus: 0 };
      const newBonus = Math.max(0, (current.bonus || 0) - amount);
      client.db.invites.set(interaction.guild.id, user.id, { bonus: newBonus });
    }

    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.check} Invites Removed\n-# Requested by ${interaction.user.displayName} • <t:${Math.floor(Date.now() / 1000)}:t>`))
      .addSeparatorComponents(new SeparatorBuilder())
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`${emoji.wickarrow} Removed **${amount}** ${type} invites from **${user.displayName}**`));
  ...
  async execute(message, args, client) {
    ...
    if (args.length < 3) {
      return message.reply({ components: [new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${emoji.info} **Usage:** \`removeinvites <user> <amount> <total|fake>\``))], flags: MessageFlags.IsComponentsV2 });
    }
    ...
    if (type === 'fake') {
      ...
      client.db.invites.set(message.guild.id, user.id, { fake: Math.max(0, (current.fake || 0) - amount) });
    } else {
      const current = client.db.invites.get(message.guild.id, user.id) || { invites: 0, bonus: 0 };
      client.db.invites.set(message.guild.id, user.id, { bonus: Math.max(0, (current.bonus || 0) - amount) });
    }
