...
module.exports = {
  ...
  slashOptions: [
    { name: 'channel', description: 'The welcome channel', type: 7, required: false, channel_types: [ChannelType.GuildText] }
  ],
  ...
  async slashExecute(interaction, client) {
    ...
    inviteConfig.setJoinChannel(interaction.guild.id, channel.id);
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.check} Join Channel Set\n-# Requested by ${interaction.user.displayName} • <t:${Math.floor(Date.now() / 1000)}:t>`))
      .addSeparatorComponents(new SeparatorBuilder())
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`${emoji.wickarrow} Welcome messages will be sent to <#${channel.id}>`));
  ...
  async execute(message, args, client) {
    ...
    inviteConfig.setJoinChannel(message.guild.id, channel.id);
    const container = new ContainerBuilder()
